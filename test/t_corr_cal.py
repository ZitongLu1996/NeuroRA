...
class test_corr_cal(unittest.TestCase):
    ...
    def test_bhvANDfmri_corr(self):
        ...
        corrs = bhvANDfmri_corr(bhv_data, fmri_data, sub_opt=0)
        self.assertEqual(corrs.shape[0], 11)

        corrs = bhvANDfmri_corr(bhv_data, fmri_data, sub_opt=1)
        self.assertEqual(corrs.shape[0], 10)
    ...
    def test_eegANDfmri_corr(self):
        ...
        corrs = eegANDfmri_corr(eeg_data, fmri_data, chl_opt=0, sub_opt=0)
        self.assertEqual(corrs.shape[0], 11)

        corrs = eegANDfmri_corr(eeg_data, fmri_data, chl_opt=0, sub_opt=1)
        self.assertEqual(corrs.shape[0], 10)

        corrs = eegANDfmri_corr(eeg_data, fmri_data, chl_opt=1, sub_opt=0)
        self.assertEqual(corrs.shape[0], 32)

        corrs = eegANDfmri_corr(eeg_data, fmri_data, chl_opt=1, sub_opt=1)
        self.assertEqual(corrs.shape[0], 10)
