    ...
    for sub in range(subs):
        ...
        for i in range(chls):
            for j in range(ts):
                ...
                corr_mat = np.zeros([trials, trials])
...
def stps_fmri(fmri_data, label_item, label_rf, ksize=[3, 3, 3], strides=[1, 1, 1]):
    ...
    # initialize the STPS
    stps = np.zeros([subs, 8, n_x, n_y, n_z])
    ...
    for sub in range(subs):
        ...
        # initialize the STPS for each subject
        sub_stps = np.zeros([8, n_x, n_y, n_z])
